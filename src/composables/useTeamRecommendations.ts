...
export function useTeamRecommendations(character: Character) {
  ...
  const activeTab = ref<string>(
    character.teammateRecommendations?.[0]?.name || 'teammates'
  )

  const activeTeamTab = ref<string>(
    character.teamCompositions?.[0]?.name || 'team'
  )
  ...
  const currentRecommendations = computed(() => {
    ...
    return characterRef.value.teammateRecommendations.find(section => section.name === activeTab.value)
  })
